...
# This reference version reads the whole file but only processes the first 
# line. The intent is to understand what overheads come from processing the
# data compared to just loading the interpreter etc
...
def read_size_list(du_file):
    ...
    with open(du_file) as f:
        # Read a single line for processing
        line = f.readline()
        # Read the rest of the file but don't do anything with it
        f.read()
        try:
